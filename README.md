...
### Q. 로그인 연동 후, 구글 시트/캘린더 동기화가 갑자기 안 됩니다. (또는 DB를 초기화했습니다)
> [!WARNING]
> **원인:** 로컬 개발 중 Docker 컨테이너나 로컬 DB(D1)를 껐다 켜서 초기화하면, 앱 서버가 구글로부터 받아온 백그라운드 동기화용 열쇠(`Refresh Token`)를 잃어버리게 됩니다. 하지만 구글은 이미 한 번 연동된 것을 기억하고 있으므로, 다시 로그인하더라도 동의 화면을 건너뛰고 새 열쇠를 발급해주지 않습니다.

**해결 방법:**
로그인 화면 하단에 있는 **"DB 초기화로 인해 동기화 오류가 발생하나요? 강제 재연동하기"** 링크를 클릭하여 로그인하세요. 
이 링크는 구글에 강제로 동의 화면(`prompt=consent`)을 다시 띄우도록 요청하여, 새로운 `Refresh Token`을 안전하게 재발급받고 동기화를 정상화합니다.
